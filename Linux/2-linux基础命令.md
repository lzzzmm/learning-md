...
/bin:用户二进制文件，包含二进制可执行文件。系统所有用户使用的命令都设在这里，例如：ps，ls，ping，grep等

/boot:这里存放的是启动linux时使用的一些核心文件，包括一些连接文件以及镜像文件

/dev：设备文件，存放的是linux的外部设备，在linux中访问设备的方式和访问文件的方式是系统的

/etc：存放所有系统管理所需要的配置文件和子目录

/home: 用户主目录，在linux中，每个用户都有一个自己的目录，一般该目录名是以用户的账号命名的

/lib:存放系统最基本动态连接共享库，作用类似windows里的dll文件，几乎所有用程序都需要用到这些共享库

/opt:可选的附加应用程序，给主机额外安装软件所摆放的目录。比如安装一个oracle数据库可以放在这个目录下

/root:系统管理员目录

/sbin:super user的意思，系统二进制文件，在这个目录下的linux命令通常由系统管理员使用

/usr/bin：系统用户使用的应用程序 ls ps等

/usr/sbin：超级用户使用的比较高级的管理程序和系统守护程序

/tmp：存放一些临时文件

/var：存放这不断扩充的东西，习惯将这些经常被修改的目录放在这个目录下，包括各种日志文件
